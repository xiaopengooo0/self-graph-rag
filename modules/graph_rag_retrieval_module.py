...
class GraphRAGRetrievalModule:
    ...
    def graph_rag_search(self,query: str,top_k=5)-> List[Document]:
        """
    ...
    def multi_hop_traversal(self, graph_query)-> List[GraphPath]:
        """
        多跳图遍历：这是图RAG的核心优势
        通过图结构发现隐含的知识关联
        """
    ...
    def _subgraph_to_documents(self, subgraph: KnowledgeSubgraph,
        ...
        doc = Document(
            ...
            metadata={
                ...
                "recipe_name": subgraph.central_nodes[0].get("name",
                                                             "知识子图") if subgraph.central_nodes else "知识子图"
            }
        )
        documents.append(doc)
