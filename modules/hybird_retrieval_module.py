...
class HybridRetrievalModule:
    ...
    def hybrid_search(self,query,top_k=5)  -> List[Document]:
        """
    ...
    def dual_level_retrieval(self, query, top_k):
        """
        双层检索：结合实体级和主题级检索
        """
        ...
        # 5. 转换为Document格式
        documents = []
        for result in unique_res[:top_k]:
            # 确保recipe_name字段正确设置
        ...
        return documents
    def vector_retrieval(self, query, top_k):
        pass
    ...
    def entity_level_retrieval(self, entity_keywords, top_k):
        """
        实体级检索：专注于具体实体和关系
        使用图索引的键值对结构进行检索
        """
        ...
        if len(results) < top_k:
            ...
            logger.info(f"补充检索结果: {neo4j_results}")
    ...
    def _get_node_neighbors(self, node_id, max_neighbors):
        """获取节点的邻居信息"""
        try:
    ...
    def _neo4j_entity_level_search(self, keywords, limit):
        """Neo4j补充检索"""
        results = []
    ...
    def topic_level_retrieval(self, topic_keywords: List[str], top_k: int = 5) -> List[RetrievalResult]:
        ...
        logger.info(f"共检索出实体信息{results}")
    ...
    def _neo4j_topic_level_search(self, keywords, limit):
        """Neo4j主题级检索补充"""
        results = []
